...
<Callout type={'info'}>
  ...
  источника первоначальной проблемы, из-за которой появился null. * Это также делает неясным, когда
  конкретные значения в классе или интерфейсе могут отсутствовать. * Вместо этого код должен
