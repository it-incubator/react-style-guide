...
<Callout type={'info'}>
  * В TypeScript перечисления (enum) уже нельзя изменять, они предоставляют фиксированный набор
  значений. * `const enum` - это особенность языка, связанная с оптимизацией, которая делает
  перечисление невидимым для пользователей JavaScript модуля. * По сути, `const enum` создает
  ...
  использовать эти значения в других модулях JavaScript. * Поэтому рекомендуется использовать
...
<Callout type={'info'}>
  Выводы: * Enum - одна из немногих возможностей TypeScript, которая не является расширением
  JavaScript на уровне типов. * Enum - хуже работает в качестве типов
</Callout>
